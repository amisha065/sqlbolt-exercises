...
```sql
SELECT director, count(director) as count_of_movies_directed FROM movies
group by director;
...
```sql
SELECT m.director, SUM(bo.domestic_sales + bo.international_sales)/1e6 AS total_sales_in_millions
FROM movies AS m
